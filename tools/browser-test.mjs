...
ok(/Pehle lab/.test(hiCh.labels),
   'the lab-first plan labels are translated', hiCh.labels.slice(0, 40));
ok(/banaiye|kijiye|karo/.test(hiCh.plan),
   'and so is the plan itself, which used to stay English', hiCh.plan.slice(0, 50));
ok(/aata hai|sahyogi|hai/.test(hiCh.body), 'the chapter body reads in Hinglish');
ok(!hiCh.notice, 'and the English-only notice is gone, because it is no longer true');
await boot(navp.page, '#/appendix');
const hiAp = await navp.page.evaluate(() => ({
  fallback: document.body.innerText.includes('NOTHING HERE IS TRANSLATED'),
...
ok((await navp.page.evaluate(() =>
     [...document.querySelectorAll('.planlist dt')].map(n => n.textContent).join(' ')))
   .includes('Lab first'),
   'and English still reads in English');
