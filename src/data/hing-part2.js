...
Object.assign(window.HING = window.HING || {}, {
  ...
  'Because the failures do not announce themselves. They arrive wrapped in an apology, or inside a code block, or with a trailing comma, or — worst — perfectly formed with an invented value inside. Now do the multiplication before the next paragraph does it for you: ten thousand requests a day at 97 percent.':
    'Kyunki failures khud ka elaan nahi karte. Woh maafi ke saath aate hain, ya code block ke andar, ya ek extra comma ke saath, ya — sabse bura — bilkul sahi shape mein lekin andar ek bani hui value ke saath. Ab agla paragraph aapke liye maths kare, usse pehle khud kijiye: das hazaar requests roz, 97 percent par.',
  ...
  'Change <code>rag_answer</code> to return a record instead of a paragraph: <code>answer</code>, <code>found</code> (boolean), <code>source_chunk_ids</code> (array), <code>supporting_quote</code>. Then re-run the three set questions.':
  ...
  '<strong>Reason one: the meter did not move.</strong> This one is arithmetic, and people skip it because it is boring. A large request costs what a large request costs, on every query, from every user, forever. Retrieval is not a workaround for a size limit. It is the thing that keeps the bill finite.':
    '<strong>Wajah ek: meter nahi hila.</strong> Yeh maths hai, aur log ise chhod dete hain kyunki yeh boring hai. Badi request ki jitni cost hai utni hi rehti hai, har query par, har user se, hamesha. Retrieval size limit ka jugaad nahi hai. Wahi bill ko seemit rakhta hai.',
  ...
  'And the thing this chapter finally lets you say exactly: when someone says their assistant “remembers” a user, ask where that memory physically lives. It is a store you built, re-sent on every message, and paid for every time.':
  ...
  'From your own product: name two tasks that would really get better with thinking, and two that would get slower and more expensive with no gain whatsoever.':
  ...
  'It pays on multi-step logic, arithmetic where each stage depends on the last, code, planning, and genuine ambiguity that needs resolving. It wastes on looking things up, pulling fields out of a document, sorting things into categories, formatting, routing, and summarising a passage you handed it.':
    'Yeh multi-step logic, aise maths jahan har charan pichhle par tika ho, code, planning, aur asli dhundhlahat sulajhane par wasool hota hai. Yeh cheezein dhoondhne, document se fields nikaalne, categories mein baantne, formatting, routing, aur aapke diye gaye passage ka summary banane par barbaad hota hai.',
  ...
  'A strong rule routes on something you can detect <em>before</em> answering — the kind of question, how many things it mentions, whether arithmetic is involved, whether retrieval came back with conflicting pieces — rather than on a guess about difficulty. And it names what would falsify it: accuracy on the no-thinking group falling below the thinking group on the same questions, or waiting time on the thinking group exceeding what the screen can absorb. A rule with no falsifying measurement is a preference wearing a rule’s clothes.':
    'Achcha niyam kisi aisi cheez par route karta hai jise aap jawaab dene se <em>pehle</em> pehchan sakte hain — sawaal ki kism, usme kitni cheezein zikr hui hain, maths shaamil hai ya nahi, retrieval se aapas mein takrane wale tukde aaye ya nahi — na ki mushkilaai ke andaaze par. Aur woh batata hai ki use kya jhoothla dega: bina-thinking wale group ki accuracy unhi sawaalon par thinking wale group se neeche girna, ya thinking wale group ka intezaar screen ki sehne ki hadd se bahar jaana. Jis niyam ko jhoothlane ki koi maap nahi, woh niyam ke kapdon mein pasand hai.',
  'Step 1 — Two Tasks, Two Settings, Four Cells':
    'Step 1 — Do kaam, do settings, chaar khaane',
  'New notebook <code>chapter-11</code>. Pick a reasoning-capable model from build.nvidia.com. Build two tasks from your own domain: one pure lookup, one multi-step (an eligibility calculation with conditions, a reconciliation across three figures).':
  ...
  'A system becomes dangerous when it has all three of: access to private data, exposure to text somebody outside your company can influence, and a way to send something outward. Any two are usually survivable. All three, and a successful instruction hidden in a document can read your data and post it somewhere.':
    'System sach mein khatarnak tab hota hai jab uske paas teenon hon: private data tak pahunch, aise text ka saamna jise aapki company ke bahar ka koi banda par asar daal sakta hai, aur kuchh bahar bhejne ka raasta. Koi bhi do aam taur par jhele ja sakte hain. Teenon hon, to kisi document mein chhupa ek kaamyaab instruction aapka data padhkar kahin bhej sakta hai.',
  ...
  'What really helps, none of it perfect and all of it worth doing: never grant all three at once; require a person to approve anything irreversible; give the system the narrowest access that still works; log what it did so you can find out afterwards; and treat every retrieved document as untrusted, because it is.':
