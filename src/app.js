...
async function resetEverything(){
  const synced = !!(window.SYNC && window.SYNC.gh && window.SYNC.gh.config().gistId);
  const msg = synced
    ? 'Reset ALL progress, notes, marks and predictions — on this device and in your synced copy?\n\n' +
      'Clearing the synced copy too is what stops it coming back on the next load. This cannot be undone.'
  ...
  if(synced){
    ...
    catch(e){ alert('Cleared on this device, but the synced copy could not be reached:\n' +
                    e.message + '\n\nIt will overwrite this device again on the next load ' +
                    'unless you reset once more while online.'); }
  }
  location.reload();
}
