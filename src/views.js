...
function data(){
  ...
  sec4.appendChild(h('div',{style:'display:flex;gap:.5rem;flex-wrap:wrap'},[
    /* Calls the shared reset rather than clearing localStorage here. Clearing
       only the device looks like it worked and then the account restores the
       lot on the next load, which is exactly the bug this button had. */
    h('button',{class:'red',onclick:()=>window.RESET_EVERYTHING()},
      'Erase all progress')]));
  w.appendChild(sec4);
  return w;
}
